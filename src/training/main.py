...
import transformations as myTransforms
import visualization

__author__ = "Ayush Goel"
...
def val(args, val_loader, model, criterion):
    ...
    iouEvalVal = evaluation.iouEval( args.classes )
...
def train(args, train_loader, model, criterion, optimizer, epoch):
    ...
    iouEvalTrain = evaluation.iouEval( args.classes )
...
def train_and_validate_segmentation(args):
    '''
    ...
    if not os.path.isfile(args.cached_data_file):
        dataLoad = dataloader.LoadData(args.data_dir, args.classes, args.cached_data_file)
    ...
    # load the model
    if not args.decoder:
        model = network.ESPNet_Encoder(args.classes, p=p, q=q)
        args.savedir = args.savedir + '_enc_' + str(p) + '_' + str(q) + '/'
    else:
        model = network.ESPNet(args.classes, p=p, q=q, encoderFile=args.pretrained)
    ...
    if args.visualizeNet:
        ...
        y = model.forward(x)
        g = visualization.make_dot(y)
        g.render(args.savedir + 'model.png', view=False)

    total_paramters = network_parameters(model)
    ...
    trainLoader = torch.utils.data.DataLoader(
        datasetloader.MyDataset(data['trainIm'], data['trainAnnot'], transform=trainDataset_main),
        batch_size=args.batch_size + 2, shuffle=True, num_workers=args.num_workers, pin_memory=True)

    trainLoader_scale1 = torch.utils.data.DataLoader(
        datasetloader.MyDataset(data['trainIm'], data['trainAnnot'], transform=trainDataset_scale1),
        batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True)

    trainLoader_scale2 = torch.utils.data.DataLoader(
        datasetloader.MyDataset(data['trainIm'], data['trainAnnot'], transform=trainDataset_scale2),
        batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True)

    trainLoader_scale3 = torch.utils.data.DataLoader(
        datasetloader.MyDataset(data['trainIm'], data['trainAnnot'], transform=trainDataset_scale3),
    ...
    trainLoader_scale4 = torch.utils.data.DataLoader(
        datasetloader.MyDataset(data['trainIm'], data['trainAnnot'], transform=trainDataset_scale4),
        batch_size=args.batch_size + 4, shuffle=True, num_workers=args.num_workers, pin_memory=True)

    valLoader = torch.utils.data.DataLoader(
        datasetloader.MyDataset(data['valIm'], data['valAnnot'], transform=valDataset),
...
if __name__ == '__main__':
    ...
    # Train and validate segmentation
    train_and_validate_segmentation(args)